            ...
            async with ClientSession(read, write) as session:
                ...
                if response.get("status") == "ok":
                    ...
                    if not memories:
                        ...
                        for m in memories:
                            print(f"   {Colors.CYAN}•{Colors.END} {Colors.BOLD}{m['id']}{Colors.END}")
                            print(f"      Nom: {m['name']}")
    ...
    try:
        async with sse_client(f"{BASE_URL}/sse", headers=headers) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                result = await session.call_tool("memory_graph", {
                    "memory_id": memory_id,
                    "format": "full"
                })
                response = json.loads(result.content[0].text)
...
def display_text(memory_id: str, nodes: list, edges: list):
    ...
    for entity_type in sorted(entities_by_type.keys()):
        ...
        for e in sorted(entities, key=lambda x: -x.get("mentions", 0)):
            ...
            desc = f" - {e['description'][:50]}..." if e.get("description") and len(e.get("description", "")) > 50 else (f" - {e['description']}" if e.get("description") else "")
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualiser le graphe d'une mémoire MCP"
    )
    parser.add_argument(
        "memory_id",
        nargs="?",
        help="ID de la mémoire à visualiser"
    )
    parser.add_argument(
        "--list", "-l",
        action="store_true",
        help="Liste les mémoires disponibles"
    )
    parser.add_argument(
        "--format", "-f",
